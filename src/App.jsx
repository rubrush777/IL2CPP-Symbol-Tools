...
function generateFridaScript(symbolMap) {
  ...
  for (const line of lines) {
    ...
    // Support: "address name" or "name address" or "name=address"
    const parts = trimmed.split(/[\s=,\t]+/);
    if (parts.length >= 2) {
      const [a, b] = parts;
      const isHex = (s) => /^0x[0-9a-fA-F]+$/.test(s) || /^[0-9a-fA-F]{8,}$/.test(s);
      if (isHex(a)) entries.push({ addr: a, name: b });
      else if (isHex(b)) entries.push({ addr: b, name: a });
    }
  }
  ...
  const hooks = entries
    .filter(e => e.name && !e.name.startsWith("_Z") || e.name.includes("il2cpp") || entries.length < 50)
    .slice(0, Math.min(entries.length, 100))
    .map(({ addr, name }) => {
      const safeAddr = addr.startsWith("0x") ? addr : "0x" + addr;
      return `
  // ${name}
  try {
    Interceptor.attach(ptr("${safeAddr}"), {
  ...
  } catch(e) { console.error("[-] Failed to hook ${name}:", e.message); }`;
    })
    .join("\n");

  return `// Frida Bridge — generated by IL2CPP Symbol Tools
// Symbols: ${entries.length} | Hooks: ${Math.min(entries.length, 100)}
...
${hooks}
  ...
  console.log("[*] Bridge initialized — ${Math.min(entries.length, 100)} hooks active");
});`;
}
